...
class PairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        w_path = self.paths[index]['w_path']

        w = np.load(w_path)
        img_w = torch.from_numpy(w)
        img_w = adaptive_avg_pool(img_w)
...
adaptive_pool = torch.nn.AdaptiveAvgPool1d(128)
def adaptive_avg_pool(input_tensor):
    input_tensor = input_tensor.unsqueeze(0).transpose(1, 2)
    output_tensor = adaptive_pool(input_tensor)
    output_tensor = output_tensor.transpose(1, 2).squeeze(0)
