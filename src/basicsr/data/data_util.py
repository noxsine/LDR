...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    w_folder = gt_folder.replace('target', 'weather')

    input_paths = list(scandir(input_folder))
    gt_paths = list(scandir(gt_folder))
    w_paths = list(scandir(w_folder))
    #print(w_folder)
    #print(w_paths)
    ...
    for idx in range(len(gt_paths)):
        ...
        w_path = w_paths[idx]
        w_path = osp.join(w_folder, w_path)



        paths.append(
            dict([(f'{input_key}_path', input_path),(f'w_path', w_path),
                  (f'{gt_key}_path', gt_path)])),
